...
#include </usr/local/cuda-9.0/include/cusparse_v2.h>
#include </usr/local/cuda-9.0/include/thrust/device_vector.h>
...
public:
  ...
  void memSetHost(float *v, int *r, int *c){
    ...
    if(error != cudaSuccess) printf("memSetHost cudaMemcp error: csrColIndDevice\n");
  }
  ...
  void s_s_dot(cuMatSparse &b, cuMatSparse &c){
    cusparseStatus_t status = cusparseScsrgemm(cuHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE, rows, b.cols, cols, descr, numVals, csrValDevice, csrRowPtrDevice, csrColIndDevice, b.descr, b.numVals, b.csrValDevice, b.csrRowPtrDevice, b.csrColIndDevice, c.descr, c.csrValDevice, c.csrRowPtrDevice, c.csrColIndDevice);

    if(status != CUSPARSE_STATUS_SUCCESS){
      cout << "ERROR cuMatSparse::s_s_dot cusparseScsrgemm" << endl;
    }
  ...
  void s_d_dot(cuMat &b, cuMat &c){
    ...
    cusparseStatus_t status = cusparseScsrmm(cuHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, rows, b.cols, cols, numVals, &alpha, descr, csrValDevice, csrRowPtrDevice, csrColIndDevice, b.mDevice, b.rows, &beta, c.mDevice, c.rows);
    if(status != CUSPARSE_STATUS_SUCCESS){
	...
	cout << "CUSPARSE_STATUS_NOT_INITIALIZED" << endl; break;
      case CUSPARSE_STATUS_ALLOC_FAILED:
	cout << "CUSPARSE_STATUS_ALLOC_FAILED" << endl; break;
      case CUSPARSE_STATUS_INVALID_VALUE:
	cout << "CUSPARSE_STATUS_INVALID_VALUE" << endl; break;
      case CUSPARSE_STATUS_ARCH_MISMATCH:
	cout <<" CUSPARSE_STATUS_ARCH_MISMATCH" << endl; break;
      case CUSPARSE_STATUS_EXECUTION_FAILED:
	cout << "CUSPARSE_STATUS_EXECUTION_FAILED" << endl; break;
      case CUSPARSE_STATUS_INTERNAL_ERROR:
	cout << "CUSPARSE_STATUS_INTERNAL_ERROR" << endl; break;
      case CUSPARSE_STATUS_MATRIX_TYPE_NOT_SUPPORTED:
	cout << "CUSPARSE_STATUS_MATRIX_TYPE_NOT_SUPPORTED" << endl; break;
      }
    }
    cudaThreadSynchronize();
  }
