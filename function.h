...
public:
  ...
  FunctionLinear(Variable *w, bool is Transpose = false);
  FunctionLinear(int output_size, int input_size);
  FunctionLinear(int output_size, int input_size, bool no_bias);

  PVariable forward(vector<PVariable> &inpus, vector<PVariable> &outputs);
...
public:
  ...
  FunctionEmbed();
  FunctionEmbed(int output_size, int input_size, bool noBias);
  PVariable forward(vector<PVariable> &inpus, vector<PVarialbe> &outputs);
  voi backward(cuMat &p_grad, vector<PVariable> &inputs, vector<PVariable> &outputs);
  void toHostArray();
  void fromHostArray();

private:
  friend class boost::serialization::access;
  template<class Archive> void serilize(Archive &ar, const unsigned int version){
    ar & boost::serialization::base_object<Function>(*this);
    ar & w;
    ar & b;
    ar & i1;
...
public:
  ...
  FunctionReLU();
  PVariable forward(vector<PVariable> &inputs, vector<PVariable> &outputs);
  void backward(cuMat &p_grad, vector<PVariable> &inpus, vector<PVariable> &outputs);
};
...
public:
  FunctionIdentity();
  PVariable forward(vector<PVariable> &inputs, vecotr<PVariable> &outputs);
  voi backward(cuMat &p_grad, vector<PVariable> &inputs, vector<PVariable> &outputs);
};
...
public:
  ...
  FunctionLSTM();
  PVariable forward(vector<PVariable> &inputs, vector<PVariable> &outputs);
  voi backward(cuMat &gh, vector<PVariable> &inputs, vector<PVariable> &outputs);
  ...
  //void joinMat(int offset, cuMat &target, cuMat &i, cuMat &f, cuMat &g, cuMat &o);
};
...
public:
  ...
  FunctionPooling(int width, int height, int deth, int windowWidth, int windowHeight, int stride, int padding);

  PVariable forward(vector<PVaraible> &inputs, vector<PVariable> &outputs);
  void backward(cuMat &gh, vector<PVariable> &inputs, vector<PVariable> &outputs);
};
