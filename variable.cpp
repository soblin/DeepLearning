...
void Variable::backward(Variable *v){
  ...
  if(v->creator != NULL){
    ...
    for(int i=0; i< v->creator->inputs.size(); i++){
      ...
      if(nv->isGetGrd) this->backward(nv.get());
    }
  }
  else{
  }
...
void Variable::unchain(){ this->creator = NULL; }

void Variable::zero_grad(){ grad.mul(0, grad); }
...
void Variable::binomial_randoms(float ratio){
  random_device rd;
  mt19937 mt(rd());
